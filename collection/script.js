...
function renderAllCategory() {
    ...
    selectElement.value = selectElement[selectElement.length - 2].value;
    renderMajorCategory();
...
function renderMajorCategory() {
    ...
    for (const key of categoryMap.keys()) {
        majorCategorySelect.add(new Option(key, key));
        modalMajorSelect.add(new Option(key, key));
    }
    renderMiddleCategory();
    renderModalMiddleCategory();
}
...
function renderMiddleCategory() {
    ...
    for (const key of categoryMap.get(majorCategorySelect.value).son.keys()) {
        middleCategorySelect.add(new Option(key, key));
    }
}

function renderModalMiddleCategory() {
    modalMiddleSelect.options.length = 1;
    for (const key of categoryMap.get(modalMajorSelect.value).son.keys()) {
        modalMiddleSelect.add(new Option(key, key));
    }
}

function renderMinorCategory() {
    modalMinorSelect.options.length = 1;
    var son = categoryMap.get(modalMajorSelect.value).son.get(modalMiddleSelect.value).son;
    if (son) {
        for (const key of son.keys()) {
            modalMinorSelect.add(new Option(key, key));
        }
    }
}
...
function beforeRenderModal(item) {
    if (item) {
        modalMajorSelect.value = item.majorCategory;
        renderModalMiddleCategory();
...
modalMajorSelect.addEventListener("change", renderModalMiddleCategory);
modalMiddleSelect.addEventListener("change", renderMinorCategory);